...
function groupObjects(items) {
    const categoryHash = {}; // Step 1

    // Step 2: Loop through the array of objects
    for (const item of items) {
        const category = item.category.toLowerCase(); // Step 3

        // Step 4: Check if the category key exists in the hash table
        if (categoryHash.hasOwnProperty(category)) {
            categoryHash[category].push(item); // Append the object
        } else {
            categoryHash[category] = [item]; // Create a new key with an array
        }
    }

    return categoryHash; // Step 5
}
